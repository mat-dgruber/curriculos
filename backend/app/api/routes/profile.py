...
CV_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "..", "..", "..", "storage", "cv"
)
...
def _profile_to_read(profile: CandidateProfile) -> CandidateProfileRead:
    return CandidateProfileRead(
        ...
        preferred_locations=(
            json.loads(profile.preferred_locations)
...
async def create_profile(
    ...
    new_profile = CandidateProfile(
        ...
        target_roles=(
            json.dumps(body.target_roles) if body.target_roles else json.dumps([])
        ),
        preferred_locations=(
...
async def get_cv_suggestions(db: AsyncSession = Depends(get_db)):
    ...
    if not text.strip():
        ...
        existing_roles = (
            json.loads(profile.target_roles) if profile.target_roles else []
        )
        existing_locations = (
            json.loads(profile.preferred_locations)
            if profile.preferred_locations
    ...
    if matched_cluster:
        ...
        clean_role = re.sub(
            r"\b(júnior|pleno|sênior|jr|sr|pl|intern|estagiário|estagiario)\b",
            "",
            role_base,
            flags=re.IGNORECASE,
        ).strip()
        ...
        roles = [
            role_base,
            (
                f"Analista de {clean_role}"
                if not clean_role.lower().startswith(
                    ("analista", "auxiliar", "assistente", "gerente", "coordenador")
                )
                else f"{clean_role} Sênior"
            ),
            (
                f"Assistente de {clean_role}"
                if not clean_role.lower().startswith(
                    ("assistente", "auxiliar", "gerente")
                )
                else f"{clean_role} Pleno"
            ),
            (
                f"Consultor de {clean_role}"
                if not clean_role.lower().startswith(("consultor", "gerente"))
                else f"{clean_role} Sênior"
            ),
            f"{clean_role} Sênior",
            f"{clean_role} Pleno",
    ...
    if profile_city_detected and profile_state_detected:
        if not cluster_found:
            matched_locations.append(
                f"{profile_city_detected}, {profile_state_detected}"
            )
